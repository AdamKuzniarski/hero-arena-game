...
export function rollDamage(attacker) {
  const base = clamp(randInt(attacker.baseDmg - 1, attacker.baseDmg + 1), 0, 999)
  let dmg = Math.max(1, base)
  if (attacker.equipped instanceof Weapon && Math.random() < attacker.equipped.crit) dmg *= 2
  ...
  for (const eff of target.effects) {
    switch (eff.type) {
      ...
      case 'regen': {
        ...
        target.hp = clamp(target.hp + eff.amount, 0, target.maxHp)
        const healed = target.hp - before
        if (healed > 0) {
          logFn(`🌿 ${target.name} regeneriert ${healed} HP.`)
        }
        eff.duration -= 1
        break
      }
    ...
    if (eff.duration > 0 && target.alive) {
      remaining.push(eff)
    }
  }
...
export function lootFor(enemy) {
  ...
  if (Math.random() < 0.35)
    drops.push(
      new Weapon(['Keule', 'Säbel', 'Speer'][randInt(0, 2)], randInt(1, 4), Math.random() * 0.2),
    )
  return drops
}
